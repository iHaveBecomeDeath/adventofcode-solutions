...
function loopThroughInstructions(instructions){
    ...
    while (i < instructions.length) {
        ...
        if (visitedInstructions.indexOf(i) != -1) {
            // console.log('halted at ', i, ' with accumulator at ', accumulator);
            break;
        }

        visitedInstructions.push(i);

        if ((currentInstruction[0] == 'nop') || (currentInstruction[0] == 'jmp' && currentInstruction[1] == '+1')) {
            i++;
        } else if (currentInstruction[0] == 'jmp') {
            i += (currentInstruction[1] * 1);
        } else if (currentInstruction[0] == 'acc') {
            accumulator += (currentInstruction[1] * 1);
            i++;
        }
    }
...
var nopjmpInstructions = instr.reduce((acc, cur, index) => {
    if (cur.substr(0, 3) == 'jmp' || cur.substr(0, 3) == 'nop') {
        acc.push(index);
    }
    return acc;
}, []);

for (var i = 0; i < nopjmpInstructions.length; i++) {
    var newInstructions = input.split(',')
    var isJmp = newInstructions[nopjmpInstructions[i]].substr(0,3) == 'jmp';
    var searchText = isJmp ? 'jmp' : 'nop';
    var replaceText = isJmp ? 'nop' : 'jmp';

    newInstructions[nopjmpInstructions[i]] = newInstructions[nopjmpInstructions[i]].replace(searchText, replaceText);

    var result = loopThroughInstructions(newInstructions);

    if (result[0] >= newInstructions.length){
        console.log('found success? at', result[0], ' with accumulator ', result[1]);
        break;
    }
}
