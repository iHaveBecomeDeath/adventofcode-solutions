...
for (var i = 0; i < passwordObjects.length; i++){
    ...
    var requiredPositions = passwordEntities[0].split('-');
    var requiredLetter = passwordEntities[1].slice(0, passwordEntities[1].length-1);
    var passwordText = passwordEntities[2];

    if ((passwordText.substr(requiredPositions[0]-1, 1) == requiredLetter)
        != 
        (passwordText.substr(requiredPositions[1]-1, 1) == requiredLetter)) {
            validCount++;
    }
}
