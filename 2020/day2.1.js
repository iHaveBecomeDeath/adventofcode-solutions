var input = `1-5 k: kkkkhkkkkkkkkkk
...
var validCount = 0;
var passwordObjects = input.split('\n');

for (var i = 0; i < passwordObjects.length; i++){
    var passwordEntities = passwordObjects[i].split(' ');
    var requiredRange = passwordEntities[0].split('-');
    var requiredLetter = passwordEntities[1].slice(0, passwordEntities[1].length-1);
    var passwordText = passwordEntities[2];

    var matches = passwordText.match(new RegExp(`${requiredLetter}`, 'g'));
    // console.log('looking for', requiredLetter, ' in ', passwordText,  requiredRange, ' found ', matches ? matches.length : 0);
    
    if (matches && (requiredRange[0] <= matches.length && matches.length <= requiredRange[1]))
    {
        validCount++;
    }
}

console.log(validCount, ' of the passwords were valid');
