...
function NumberOfOccupiedSeatsInDirection(allRows, startRowIndex, startSeatIndex, rowMod, seatMod) {
    var incr = 1;
    var occupiedCount = 0;

    while (true) {
        var curRowIdx = startRowIndex + (rowMod * incr);
        var curSeatIdx = startSeatIndex + (seatMod * incr);
        if (curRowIdx < 0 || curRowIdx >= allRows.length) {
            break;
        }
        if (curSeatIdx < 0 || curSeatIdx >= allRows[curRowIdx].length) {
            break;
        }

        var val = allRows[curRowIdx][curSeatIdx];
        if (!val) {
            break;
        }
...
function GetLineOfSightOccupiedSeats(arrayOfRows, rowIndex, seatIndex) {
    var total = 0;

    total += NumberOfOccupiedSeatsInDirection(arrayOfRows, rowIndex, seatIndex, -1, -1); // up left
    total += NumberOfOccupiedSeatsInDirection(arrayOfRows, rowIndex, seatIndex, -1, 0); // up
    total += NumberOfOccupiedSeatsInDirection(arrayOfRows, rowIndex, seatIndex, -1, 1); // up right
    total += NumberOfOccupiedSeatsInDirection(arrayOfRows, rowIndex, seatIndex, 0, -1); // left
    total += NumberOfOccupiedSeatsInDirection(arrayOfRows, rowIndex, seatIndex, 0, 1); // right
    total += NumberOfOccupiedSeatsInDirection(arrayOfRows, rowIndex, seatIndex, 1, -1); // down left
    total += NumberOfOccupiedSeatsInDirection(arrayOfRows, rowIndex, seatIndex, 1, 0); // down
    total += NumberOfOccupiedSeatsInDirection(arrayOfRows, rowIndex, seatIndex, 1, 1); // down right
...
function LoopThroughRows(rowsToLoop) {
    ...
    for (var i = 0; i < rowsToLoop.length; i++) {
        ...
        for (var r = 0; r < row.length; r++) {
            var occupiedCount = GetLineOfSightOccupiedSeats(rowsToLoop, i, r);
            var newLetter = row[r];

            if (occupiedCount == 0 && row[r] == 'L') {
                newLetter = '#';
                changedSeats++;
            }
            if (occupiedCount >= 5 && row[r] == '#') {
                newLetter = 'L';
                changedSeats++;
            }
            newRow += newLetter;
