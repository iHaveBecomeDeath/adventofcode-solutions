...
entries.forEach(element => {
    var rowHints = element.slice(0, 7);
    var seatHints = element.slice(-3);
    var currentRows = availableRows;

    var ri = 0;
    while (currentRows.length > 1)
    {
        var rh = rowHints[ri];
        if (rh == 'F') {
            currentRows = currentRows.slice(0, currentRows.length / 2)
        } else if (rh == 'B') {
            currentRows = currentRows.slice(-currentRows.length / 2)
        }
        ri++;
    }
    var rowIndex = currentRows.toString();

    results[0].push(rowIndex);

    var currentSeats = availableSeats;
    var si = 0;
    while (currentSeats.length > 1)
    {
        var sh = seatHints[si];
        if (sh == 'L') {
            currentSeats = currentSeats.slice(0, currentSeats.length / 2)
        } else if (sh == 'R') {
            currentSeats = currentSeats.slice(-currentSeats.length / 2)
        }
        si++;
    }

    var seatIndex = currentSeats.toString();
    results[1].push(seatIndex);
});

var highRow = ''+Math.max(...results[0]);
var highIdx = results[0].indexOf(highRow);
var highSeatNo = results[1][highIdx];
console.log((highRow*8)+1*highSeatNo, ' was the highest seat identifier in this batch');