...
function ChangeDirection(leftRight){
    currentX = waypointPosition.x;
    currentY = waypointPosition.y;
    if (leftRight == 'left'){
        waypointPosition.x = currentY *-1;
        waypointPosition.y = currentX;
    } else {
        waypointPosition.x = currentY;
        waypointPosition.y = currentX * -1;
    }
}
    ...
    switch (action) {
        ...
        case 'F':
            for (var j = 0; j < number; j++) {
                position.x += waypointPosition.x;
                position.y += waypointPosition.y;
            }
            break;
        case 'L':
            if (number == 180) {
