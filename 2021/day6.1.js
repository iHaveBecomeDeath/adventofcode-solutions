...
for (var day = 0; day < 80; day++) {
  var zeroes = fishInTheSea.filter(val => val == 0);
  var others = fishInTheSea.filter(val => val > 0);
  var newborns = zeroes.map(z => 8);

  fishInTheSea = zeroes.map(z => 6).concat(others.map(o => o - 1)).concat(newborns);
  
}

console.log(fishInTheSea.length);
