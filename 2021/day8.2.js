var input = `badf gadfec bgcad ad dbcfg gcaeb fecdgab gad bgcadf efcdgb | gcadb ad agd deacfg
...
var digitLines = input.split('\n').map(l => l.split(' | '));

var sum = 0;
var test = '0';
for (var i = 0; i < digitLines.length; i++) {
  var signals = digitLines[i][0].split(' ');
  var digits = digitLines[i][1].split(' ');
  //console.log(signals, digits);

  var joinedSignals = signals.join('').split('').sort();

  var occurences = {};
  for (var c = 0; c < joinedSignals.length; c++) {
      var char = joinedSignals[c];
      if (occurences[char]) {
        occurences[char]++;
      } else {
        occurences[char] = 1;
  ...
  var entries = Object.entries(occurences);
  var seven = signals.filter(s => s.length == 3)[0];
  var one = signals.filter(s => s.length == 2)[0];
  var inSevenButNotOne = seven.split('').filter(sv => !one.includes(sv))[0];
  var four = signals.filter(s => s.length == 4)[0];

  var charMap = {
    a: inSevenButNotOne,
    b: entries.filter(([key, value]) => value == 6)[0][0],
    c: entries.filter(([key, value]) => value == 8 && key != inSevenButNotOne)[0][0],
    d: entries.filter(([key, value]) => value == 7 && !four.includes(key))[0][0],
    e: entries.filter(([key, value]) => value == 4)[0][0],
    f: entries.filter(([key, value]) => value == 9)[0][0],
  }
  var allChars = ['a', 'b', 'c', 'd', 'e', 'f', 'g'];
  var usedChars = [charMap.a, charMap.b, charMap.c, charMap.d, charMap.e, charMap.f];
  charMap.g = allChars.filter(ac => !usedChars.includes(ac))[0];
  console.log(charMap);
  ...
  for (var d = 0; d < digits.length; d++) {
    ...
    if (dg.length == 2) {
      ...
      if (dg.includes(charMap.e)) {
        lineSum += '2';
      } else if (dg.includes(charMap.c)) {
        lineSum += '3';
      } else {
        lineSum += '5';
      }
      ...
      console.log('six');
      if (!dg.includes(charMap.d)) {
        lineSum += '0';
        console.log('became zero');
      } else if (dg.includes(charMap.e)) {
        lineSum += '6';
        console.log('became six');
      } else {
        lineSum += '9';
        console.log('became nine');
      }
    }
  }
  console.log(lineSum, sum);
  sum += Number(lineSum);
}
