...
input.forEach(row => {
  var sortedRow = row.sort((a, b) => a + b);
  var sortedRowLength = sortedRow.length;
  for (var i = 0; i < sortedRowLength; i++) {
    var foundMatch = sortedRow.filter(a => a !== sortedRow[i] && a % sortedRow[i] === 0);
    sum += foundMatch[0] ? foundMatch[0] / sortedRow[i] : 0;
  }
});
